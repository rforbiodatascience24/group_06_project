...
```{r}
df_intensities_long <- df_intensities |> 
  pivot_longer(
...
```{r}
df_intensities_long <- df_intensities_long |> 
...
```{r}
sample_n(df_intensities_long,10)
...
data_path <- here("data")
file_name <- "02_df_intensities_long.tsv.gz"
write_tsv(df_intensities_long, str_c(data_path, file_name,sep = "/"))
