...
df_test_keywords <- df_uniprot_mouse |> 
  filter(str_detect(`Gene Names`, 'Ckap5')) |> 
  pull(Keywords) |> 
  str_c(collapse ='; ')
print(Keywords)
```
...
```{r}
df_type_summ2 <- df_type2 |> 
    group_by(protein_groups, cell_type) |> 
    summarize(non_na_count = sum(!is.na(intensity))) |> 
    ungroup()
```

```{r}
# make df with transcription regulation proteins

df_tf <- df_type2 |> 
  filter(str_detect(description_column, "Transcription regulation")) |> 
  group_by(cell_type) |>
  nest() |> 
  mutate(nr_protein_groups = map(.x = data,
                                 .f = ~n_distinct(pluck(.x, "protein_groups"))))
...
```{r}
# simple bar chart with total number pgs
