...
## Creating a test data set in wide format based on means only 

This test dataset makes it possible for us to plot volcano plots and histograms. But we don't think it is the most elegant format. It would be better if we could keep the long format with all replicates, but we experienced difficulty in then creating a 2logfold variable and a p.value variable for each two cell-types that are compared. The reason we experienced this difficulty is because we could not find a tidyverse method to make these new variables columns.

```{r}
#In order to plot we use the mean_intensities data frame
#We pivot wider to be able to create gmp and hsc columns, but this is not an elegant solution... 
...
pivot_wider(names_from = cell_type, values_from = c(mean, sd)) |> 
  ...
  #Here we make a fold column. But should the fold be created by division or substratction? 
  mutate(fold = mean_gmp/mean_hsc) |> 
  #Using our pvalue function
...
Creating a function that finds the p-value based on the two sample students t.test. Propably the function should end up going to its own document. This function is used for the volcano plots.
