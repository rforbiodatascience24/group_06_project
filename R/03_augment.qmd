...
df_no_na_long <- df_norm_long |> 
  ...
  mutate(intensity = case_when(
    ...
    #is.na(intensity) ~ mean(intensity, na.rm = TRUE) - 2.326 * sd(intensity, na.rm = TRUE),
...
```{r}
df_no_na_wide <- pivot_wider(data = df_no_na_long, id_cols = 'cell_type',
                             names_from = 'protein_groups', values_from = 'intensity')
```
...
summary(df_no_na_long)

```

```{r}
# Produce file 03_dat_aug.tsv

project_path = here()
setwd(project_path)
write_tsv(df_no_na_long, "data/03_dat_aug.tsv")
```
